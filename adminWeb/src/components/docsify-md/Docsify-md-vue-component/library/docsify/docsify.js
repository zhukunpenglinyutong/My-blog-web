  ...
  (function () {
    ...
    */
    ...
    function callHook(vm, hook, data, next) {
      // console.log('进来了 hook, data, next', hook, data, next)
    ...
    ********************************************** */
    ...
    var Compiler = function Compiler(config, router) {
      ...
      // console.log('mdConf', mdConf)
  
      if (isFn(mdConf)) { // 判断 mdConf 是否是函数，这里一个是对象
        // console.log('1')
        compile = mdConf(marked, renderer);
      } else {
        // console.log('进到这里面来了')，这里应该是核心的设置部分
        // console.log('merge(renderer, mdConf.renderer)', merge(renderer, mdConf.renderer))
        // 这里 注释了也没事，应该属于 marked的那啥部分
        marked.setOptions(
          merge(mdConf, {
      ...
      this.compile = function (text) {
        ...
        var result = cached(function (_) {
          ...
          // isPrimitive(text) 是 string 或者 number 吗
          // console.log('isPrimitive(text)', isPrimitive(text))
          if (isPrimitive(text)) { // 是，表示是右侧 栏目页
            html = compile(text);
          } else { // 不是表示是文章内容区域
            // 重要 🔥，这里 text 好像就是 md 数据源
            html = compile.parser(text);
          }
          
          // emojify(html) 这个函数不知道是什么意思，但是感觉结果一样
          html = config.noEmoji ? html : emojify(html);
          
          slugify.clear(); // 这是什么意思？？？
          // slugify.clear = function () {
          //   cache$1 = {};
          // };
        ...
        // console.log('拿到了 HTML结果 result', result)
        return result
      };
    };
    ...
    function prerenderEmbed(ref, done) {
      ...
      walkFetchEmbed({compile: compile, embedTokens: embedTokens, fetch: fetch}, function (ref) {
        ...
        if (token) {
          ...
          // 进到这里面来了
          // console.log('tokens.concat()', tokens.concat())
          // console.log('cached$1', cached$1)
  
          let nei = tokens.concat()
          // console.log('nei', nei)
    ...
    function initRender(vm) {
      ...
      // Init markdown compiler 初始化markdown编译器
      // console.log('new Compiler(config, vm.router);', new Compiler(config, vm.router))
    ...
    History.prototype.getFile = function getFile (path, isRelative) {
      ...
      // 这个地方竟然循环遍历 ？？？| 这代码优化
      // console.log('测试 getFile path', path)
      return path
    };
    ...
    var HashHistory = (function (History$$1) {
      ...
      HashHistory.prototype.parse = function parse (path) {
        ...
        // console.log('触发', parseQuery(query))
        return {
          path: path,
    ...
    function updateRender(vm) {
      vm.router.normalize();
      // console.log('vm.router.parse()', vm.router.parse())
    ...
    function initMixin(proto) {
      proto._init = function () {
        ...
        callHook(vm, 'init');
        initRouter(vm); // Add router这里主要为编译层，初始化markdown编译器
  
        initRender(vm); // Render base DOM 这里主要为编译层，初始化markdown编译器
        // console.log('vm2', vm)
